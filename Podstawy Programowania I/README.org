...
\end{cases}$$
#+END_LATEX
#+HTML: <img src="https://latex.codecogs.com/png.image?\inline&space;\large&space;\dpi{120}\bg{white}$$y(x)=\begin{cases}\frac{3}{4}*x^{3}*\frac{7}{8}x&\text{dla&space;x[-10,0]}\\\frac{1}{sin(x)&plus;3}&\text{dla&space;x(0,4]}\\-2x/3&\text{dla&space;pozostalych}\end{cases}$$" alt="LaTeX"\>
